...
const MediaEdition = ({ show, settings, onChange, onCancel }: Props) => {
  ...
  if(show) {
    ...
    return (
      <div style={style} className="mediaEdition">
        ...
        <div>
          <button style={{ ...buttonStyle, ...leftStyle }} onClick={onCancel}>CANCEL</button>
          <button style={{ ...buttonStyle, ...rightStyle }} onClick={onSet}>SAVE</button>
        </div>
      </div>
    );
  }
